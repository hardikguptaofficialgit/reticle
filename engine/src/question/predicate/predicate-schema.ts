...
const PREDICATE_ALIASES: Readonly<Record<string, Readonly<Record<string, string>>>> = {
  ...
  // `query` follows from `element`'s nested locator — the spelling agents reach for after writing
  // `reticle_query`. All three were hard rejections, and a rejected predicate produces NO verdict at
  // all — the drive ends with nothing rather than with a failure, which is the worst outcome of the
  // three. (#1001.1)
